...
import { useTheme } from '@aux/components/core/useTheme.js'
import { navConfig } from './useNav.js'
...
function isItemActive(item) {
  ...
  return (
    item.children?.some((child) => {
      if (child.path) return current === item.path + '/' + child.path
      return false
    }) ?? false
  )
}
...
function isChildActive(item, child) {
  const current = route.path.replace(/\.html$/, '')
  if (child.path) return current === item.path + '/' + child.path
  return current === item.path && route.data?.frontmatter?.hash === child.hash
}
...
// Ensure data-theme is set on initial load
onMounted(() => {
...
<template>
  <div class="flex min-h-screen bg-base-app text-base-normal">
                  ...
                  <a
                    ...
                    :class="
                      isChildActive(item, child)
                        ...
                        : 'text-base-dim hover:text-base-normal'
                    "
                    @click="(e) => navigate(e, childLink(item, child))"
                  >
                    {{ child.name }}
                  </a>
                </div>
    ...
    <main class="flex-1 bg-base-app min-h-screen">
      <Content />
    </main>
  </div>
</template>
