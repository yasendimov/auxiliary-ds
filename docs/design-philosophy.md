...
**Operational interfaces:** WCAG AAA for critical data — altitude, heading, speed, alert status. A pilot reading these values cannot afford a contrast failure. Text on alert backgrounds (`alarm-content` on `alarm-surface`, etc.) must meet AAA in every theme.
...
```html
<div class="grid grid-cols-4 gap-y-px bg-base-ui">
  <div class="bg-base-app p-8">...</div>
  <div class="bg-base-app p-8">...</div>
</div>
```
...
Content sections are indexed in two-digit monospace: `01`, `02`, `03`. This gives interfaces a table-of-contents quality and reinforces the engineering aesthetic. Numbers use `text-base-dim` — structural markers, not content. Em-dash `—` replaces numbers for non-sequential items.

### Metadata Pattern

Version strings, section eyebrows, and meta labels follow one pattern: monospace, `text-xs`, `tracking-[0.2em]`, uppercase, `text-base-dim`. The wide tracking (positive, inverted from the standard negative) signals: this is metadata about the content, not the content itself.
...
- Current library: Heroicons (`@heroicons/vue`), imported directly from size/variant paths
