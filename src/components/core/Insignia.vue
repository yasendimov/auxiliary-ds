...
<template>
  <svg
    ...
    role="img"
    :aria-label="seed + ' insignia'"
  >
    <defs>
      <clipPath :id="insignia.clipId">
        <path v-if="!insignia.isCircle" :d="insignia.framePathD" />
    ...
    <!-- Subtle fill -->
    <path
      v-if="insignia.hasFill && !insignia.isCircle"
      :d="insignia.framePathD"
      :fill="insignia.color.step3"
      fill-opacity="0.15"
      stroke="none"
    />
    <circle
    ...
    <!-- Frame stroke -->
    <path
      v-if="!insignia.isCircle"
      :d="insignia.framePathD"
      fill="none"
      :stroke="insignia.color.step9"
      stroke-width="1.5"
    ...
    <!-- Double border (inset) -->
    <g v-if="insignia.hasDoubleBorder" transform="translate(32,32) scale(0.88) translate(-32,-32)">
      <path
        v-if="!insignia.isCircle"
        :d="insignia.framePathD"
        fill="none"
    ...
    <g :clip-path="`url(#${insignia.clipId})`">
      <!-- Decorations -->
      <template v-for="(el, i) in insignia.decorations" :key="'d' + i">
        <line
          v-if="el.type === 'line'"
          :x1="el.x1"
          :y1="el.y1"
        ...
        <path
          ...
          :d="el.d"
          :fill="el.filled ? insignia.color.step9 : 'none'"
          :stroke="el.filled ? 'none' : insignia.color.step9"
          :stroke-width="el.sw || 1.5"
          stroke-linecap="round"
          stroke-linejoin="round"
          :opacity="el.opacity || 1"
      ...
      <!-- Central device -->
      <g :transform="insignia.deviceScale !== 1 ? `translate(32,32) scale(${insignia.deviceScale}) translate(-32,-32)` : undefined">
        <template v-for="(el, i) in insignia.device" :key="'e' + i">
          <line
            v-if="el.type === 'line'"
            :x1="el.x1"
            :y1="el.y1"
